# initProps()
下面来看一下vue在第一次创建时如何初始化props中的属性，在其中，它为每一个props属性变更为响应式属性，在其更改时对其依赖项进行通知。
...
## validateProp()——效验prop的type, 并返回一个合适的值
...
### getTypeIndex(type, expectedTypes)——查看当前type是否在期望的type中
返回符合预期的`type`的下标
```js
